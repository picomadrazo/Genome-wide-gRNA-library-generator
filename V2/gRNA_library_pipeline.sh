...
if [ $# -lt 2 ]; then
    echo "Usage: $0 <GENOME_FASTA> <CDS_FASTA> <SPACER_LENGTH>"
    exit 1
fi
...
GENOME_BASE=$(basename "$GENOME_FASTA" .fasta)  # You can also adjust the extension (e.g., .fa)

# Create the directory for Bowtie index
mkdir -p "$GENOME_BASE"_index

# Build the Bowtie index
echo "Building Bowtie index for the genome..."
bowtie-build "$GENOME_FASTA" "$GENOME_BASE"_index/GENOME_index
...
python3 all_gRNAs_CDS.py "$CDS_FASTA" -l $SPACER_LENGTH
...
bowtie -v 0 -k 100 -f -x "$GENOME_BASE"_index/GENOME_index gRNA_lib/2_all_spacers_in_CDS.fasta --al gRNA_lib/3_spacers_in_CDS_and_ORF.fasta --un gRNA_lib/4_inbetween_exons_spacers.fasta
...
bowtie -n 3 --seedlen 9 -k 100 -f -x "$GENOME_BASE"_index/GENOME_index gRNA_lib/3_spacers_in_CDS_and_ORF.fasta -S gRNA_lib/bowtie_output.sam
